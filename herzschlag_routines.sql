...
/*!50001 VIEW `artist_albums` AS select `a`.`ArtistID` AS `artistid`,`a`.`Name` AS `artist_name`,`al`.`AlbumID` AS `albumid`,`al`.`Title` AS `album_title`,`al`.`ReleaseYear` AS `releaseyear` from (`artists` `a` join `albums` `al` on((`a`.`ArtistID` = `al`.`ArtistID`))) */;
